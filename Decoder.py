...
class Decoder:
    def __init__(self, key, full_path_text):
        self.key = key
        if int(key) > 159:
    ...
    # Определение метода шифрования из считывания символа в файле_шифре
        if os.path.exists(self.full_path_text):
            with open(self.full_path_text, 'r', encoding='utf-8') as file_number_methods:
    ...
    def Dec_AveMe(self):
        ...
        if os.path.exists(self.full_path_text.replace('.txt', '_recypher_AveMe.txt')):
            temp_file = self.full_path_text.replace('.txt', '_recypher_AveMe.txt')
            os.remove(temp_file)
        # Создание расшифрованного файла с названием метода
        file_recypher = self.full_path_text.replace('.txt', '_recypher_AveMe.txt')
        key = int(self.key)
        # Открытие и работа с файлом_шифром
        with open(self.full_path_text, 'r', encoding='utf-8') as file_text_cypher:
            ...
            mb.showinfo("Ок", "Текст успешно дешифрован")
    ...
    def Dec_Сaesar_Сycle(self):
        ...
        '''
        # Проверка на наличие одноименного файла_дешифра
        if os.path.exists(self.full_path_text.replace('.txt', '_recypher_Сaesar_Сycle.txt')):
            temp_file = self.full_path_text.replace('.txt', '_recypher_Сaesar_Сycle.txt')
            os.remove(temp_file)
        # Создание расшифрованного файла с именем метода
        file_recypher = self.full_path_text.replace('.txt', '_recypher_Сaesar_Сycle.txt')
    ...
    def Dec_Multi_Cucle(self):
        ...
        if os.path.exists(self.full_path_text.replace('.txt', '_recypher_Dec_Multi_Cucle.txt')):
            temp_file = self.full_path_text.replace('.txt', '_recypher_Dec_Multi_Cucle.txt')
            os.remove(temp_file)
        # Создание расшифрованного файла с названием метода
        file_recypher = self.full_path_text.replace('.txt', '_recypher_Dec_Multi_Cucle.txt')
